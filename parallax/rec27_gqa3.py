...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, head_dim=128):
        super().__init__()
        self.num_heads = dim // head_dim          # query/probe heads (H_q = 6)
        self.num_kv_heads = self.num_heads // 2    # GQA: k,v heads (H_kv = 3, 2:1 grouping)
        self.head_dim = head_dim
        hdim = self.num_heads * self.head_dim
        kvdim = self.num_kv_heads * self.head_dim  # 3*128 = 384
        self.q = Linear(dim, hdim)                 # 6 heads (full)
        self.k = Linear(dim, kvdim)                # 3 heads (GQA)
        self.v = Linear(dim, kvdim)                # 3 heads (GQA)
        self.r = Linear(dim, hdim)  # Parallax rho — 6 heads (full, matches q)
        self.proj = Linear(hdim, dim)
        self.rotary = Rotary(head_dim)

    def forward(self, x: Tensor):
        B, T = x.size(0), x.size(1)
        q = self.q(x).view(B, T, self.num_heads, self.head_dim)      # 6 heads
        k = self.k(x).view(B, T, self.num_kv_heads, self.head_dim)   # 3 heads (GQA)
        v = self.v(x).view(B, T, self.num_kv_heads, self.head_dim)   # 3 heads (GQA)
        r = self.r(x).view(B, T, self.num_heads, self.head_dim)      # 6 heads
